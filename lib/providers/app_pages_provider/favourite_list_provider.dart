...
log("DDD:$favoriteList");
    ...
    if(providerCtrl.text.isNotEmpty || serviceCtrl.text.isNotEmpty){
     ...
     if(selectedIndex == 0){
       ...
       notifyListeners();
       for (var data in searchList ) {
         if (data.service!.title!.replaceAll(" ", "")
             .toLowerCase()
      ...
      serviceFavList = [];
     try{
       favoriteList = appArray.favouriteProviderList.map((e) => FavouriteModel.fromJson(e)).toList();
       log("favoriteList :${favoriteList.length}");
...
fav.provider = data;
      int index = favoriteList
          .indexWhere((element) =>
      element.providerId.toString() == id.toString());
  ...
  deleteToFav(context, id,type) async {
    ...
    if(type == "service") {
      ...
      notifyListeners();*/log("serviceId :$id");
      favoriteList.removeWhere(
              (element) => element.serviceId.toString() == id.toString());
      serviceFavList.removeWhere(
              (element) => element.serviceId.toString() == id.toString());
log("favoriteList :${favoriteList.length}");
  ...
  onRemoveService(context, {id}) {
    if (int.parse(serviceFavList[id].service!.selectedRequiredServiceMan!) == 1) {
      route.pop(context);
    } else {
      serviceFavList[id].service!.selectedRequiredServiceMan =
          (int.parse(serviceFavList[id].service!.selectedRequiredServiceMan!) - 1).toString();
    }
    notifyListeners();
  }

  onAdd({id}) {
    int count = int.parse(serviceFavList[id].service!.selectedRequiredServiceMan!);
    count++;
